...
import de.htw.ba.ue04.facedetection.*;
...
public class FaceDetectionController extends FaceDetectionBase {
	...
	protected void calculateIntegralImage(int[] srcPixels, int width, int height) {
		...
		integralImage = new IntegralImage(srcPixels, width, height);
	}
	...
	protected void createManualClassifier() {
		...
		MyWeakClassifier weakClasifier = new MyWeakClassifier(100,60,20,30);

		strongClassifier = new StrongClassifier(avgFace.width, avgFace.height);
		strongClassifier.addWeakClassifier(weakClasifier);
	}
